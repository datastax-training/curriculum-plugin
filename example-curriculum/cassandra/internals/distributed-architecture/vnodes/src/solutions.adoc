== Exercise {solution_number}: VNodes
...
1) Be sure neither of your nodes is running. Use the `dsetool status` command to check their running status. If either is running, be sure to stop them using the `dse cassandra-stop` or `nodetool stopdaemon` command as we did earlier.
